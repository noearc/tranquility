...
describe("Pattern", function()
		...
		it(
			...
			function()
				...
				local expectedEvents = List:new({
					Event:new(TimeSpan:new(1, 2), TimeSpan:new(2, 2), 5),
				})
	...
	describe("withQuerySpan", function()
		it("should return new pattern with that modifies query span with function when queried", function()
			...
			local expectedEvents = List:new({
				Event:new(TimeSpan:new(0.5, 1.5), TimeSpan:new(0.5, 1.5), 5),
			})
	...
	describe("withEventTime", function()
		it("should return new pattern with function mapped over event times", function()
			...
			local expectedEvents = List:new({
				Event:new(TimeSpan:new(0.5, 1.5), TimeSpan:new(0.5, 1.5), 10),
			})
	...
	describe("onsetsOnly", function()
		it("should return only events where the start of the whole equals the start of the part", function()
			...
			assert.are.same(
				patternWithOnsetsOnly:query(State:new(TimeSpan:new(Fraction:new(0), Fraction:new(3)))),
				List:new({ event1 })
			)
		end)
		...
		it("pure patterns should not behave like continuous signals... they should have discrete onsets", function()
			...
			assert.are.equal(actualEvents, List:new({ expectedEvent }))
			...
			assert.are.equal(actualEvents, List:new({}))
		end)
	end)
	...
	describe("fastgap", function()
		it("should bring pattern closer together", function()
			local actualEvents = Fastcat({ Pure("bd"), Pure("sd") }):fastgap(4):firstCycle()

			local expectedEvents = List:new({
				Event:new(
					TimeSpan:new(Fraction:new(0), Fraction:new(1, 8)),
					TimeSpan:new(Fraction:new(0), Fraction:new(1, 8)),
					"bd",
					List:new(),
					false
				...
				Event:new(
					TimeSpan:new(Fraction:new(1, 8), Fraction:new(1, 4)),
					TimeSpan:new(Fraction:new(1, 8), Fraction:new(1, 4)),
					"sd",
					List:new(),
					false
	...
	describe("compress", function()
		it("should bring pattern closer together", function()
			local actualEvents = Fastcat({ Pure("bd"), Pure("sd") }):compress(1 / 4, 3 / 4):firstCycle()
			local expectedEvents = List:new({
				Event:new(
	...
	describe("timecat", function()
		it("should return a pattern based one the time-pat 'tuples' passed in", function()
			...
			local expectedEvents = List:new({
				Event:new(
					TimeSpan:new(Fraction:new(0), Fraction:new(3, 16)),
					TimeSpan:new(Fraction:new(0), Fraction:new(3, 16)),
					"bd"
				),
				Event:new(
					TimeSpan:new(Fraction:new(3, 16), Fraction:new(3, 8)),
					TimeSpan:new(Fraction:new(3, 16), Fraction:new(3, 8)),
					"bd"
				),
				Event:new(
					TimeSpan:new(Fraction:new(3, 8), Fraction:new(9, 16)),
					TimeSpan:new(Fraction:new(3, 8), Fraction:new(9, 16)),
					"bd"
				),
				Event:new(
					TimeSpan:new(Fraction:new(9, 16), Fraction:new(3, 4)),
					TimeSpan:new(Fraction:new(9, 16), Fraction:new(3, 4)),
					"bd"
				),
				Event:new(
					TimeSpan:new(Fraction:new(3, 4), Fraction:new(25, 32)),
					TimeSpan:new(Fraction:new(3, 4), Fraction:new(25, 32)),
					"hh"
				),
				Event:new(
					TimeSpan:new(Fraction:new(25, 32), Fraction:new(13, 16)),
					TimeSpan:new(Fraction:new(25, 32), Fraction:new(13, 16)),
					"hh"
				),
				Event:new(
					TimeSpan:new(Fraction:new(13, 16), Fraction:new(27, 32)),
					TimeSpan:new(Fraction:new(13, 16), Fraction:new(27, 32)),
					"hh"
				),
				Event:new(
					TimeSpan:new(Fraction:new(27, 32), Fraction:new(7, 8)),
					TimeSpan:new(Fraction:new(27, 32), Fraction:new(7, 8)),
					"hh"
				),
				Event:new(
					TimeSpan:new(Fraction:new(7, 8), Fraction:new(29, 32)),
					TimeSpan:new(Fraction:new(7, 8), Fraction:new(29, 32)),
					"hh"
				),
				Event:new(
					TimeSpan:new(Fraction:new(29, 32), Fraction:new(15, 16)),
					TimeSpan:new(Fraction:new(29, 32), Fraction:new(15, 16)),
					"hh"
				),
				Event:new(
					TimeSpan:new(Fraction:new(15, 16), Fraction:new(31, 32)),
					TimeSpan:new(Fraction:new(15, 16), Fraction:new(31, 32)),
					"hh"
				),
				Event:new(
					TimeSpan:new(Fraction:new(31, 32), Fraction:new(1)),
					TimeSpan:new(Fraction:new(31, 32), Fraction:new(1)),
					"hh"
				),
			})
